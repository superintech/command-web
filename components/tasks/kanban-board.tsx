...
const PRIORITIES = [
  { value: 'LOW', label: 'Low', color: 'bg-slate-500/20 text-[hsl(var(--text-secondary))]' },
  ...
  return (
    <ContextMenu>
      <ContextMenuTrigger>
        <Card
          className={cn(
            'cursor-pointer hover:shadow-md transition-all bg-[hsl(var(--layout-bg))] border-[hsl(var(--layout-border))]',
          ...
          <CardContent className="p-3">
              ...
              <div className="flex items-center gap-1 shrink-0">
                ...
                {onStartTimer && onStopTimer && (
                  <Button
                    ...
                    className={cn(
                      'h-6 w-6',
                      isTimerActive ? 'bg-red-500 hover:bg-red-600 animate-pulse' : 'text-[hsl(var(--text-secondary))] hover:text-[hsl(var(--text-primary))] hover:bg-[hsl(var(--layout-card-hover))]'
                    )}
            ...
            {task.description && (
              <p className="text-xs text-[hsl(var(--text-secondary))] mt-1 line-clamp-2">
                {task.description.replace(/<[^>]*>/g, '').replace(/#{1,6}\s?|(\*{1,2}|_{1,2})(.*?)\1|`{1,3}[^`]*`{1,3}|\[([^\]]*)\]\([^)]*\)|[>\-*+]|\!\[.*?\]\(.*?\)/g, '$2$3').replace(/\s+/g, ' ').trim().slice(0, 80)}
              </p>
            )}
            ...
            <div className="mt-2 flex items-center gap-3 text-xs text-[hsl(var(--text-secondary))]">
              {task.dueDate && (
                <div className={cn(
...
function KanbanColumn({
  ...
  return (
    <div
      ref={setNodeRef}
      className={cn(
        'flex flex-col rounded-xl bg-[hsl(var(--layout-card))] border border-[hsl(var(--layout-border))]/50 p-3 min-h-[500px] transition-colors',
    ...
    >
      <div className="mb-3 flex items-center gap-2">
        ...
        <span className="text-xs text-[hsl(var(--text-secondary))] bg-[hsl(var(--layout-border))] px-1.5 py-0.5 rounded">
          {tasks.length}
        </span>
      </div>
      ...
      <SortableContext items={tasks.map((t) => t.id)} strategy={verticalListSortingStrategy}>
        <div className="flex-1 space-y-2 overflow-y-auto">
          ...
          {tasks.length === 0 && (
            <div className={cn(
              'flex items-center justify-center h-24 text-sm text-[hsl(var(--text-secondary))] border-2 border-dashed border-[hsl(var(--layout-border))] rounded-lg transition-colors',

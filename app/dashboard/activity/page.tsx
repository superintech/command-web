...
export default function ActivityPage() {
  ...
  const activityItems: ActivityItem[] = [
    ...tasks.map((task, index) => ({
      id: `task-${task.id}`,
      type: 'task' as const,
      user: task.assignee?.name || task.reporter?.name || 'System',
      ...
      date: new Date((task as any).updatedAt || (task as any).createdAt || Date.now()),
      colorIndex: index,
    })),
    ...projects.map((project, index) => ({
      ...
      target: project.name,
      targetId: project.id,
      date: new Date((project as any).updatedAt || (project as any).createdAt || Date.now()),
      ...
      <div className="bg-[hsl(var(--layout-card))] rounded-xl border border-[hsl(var(--layout-border))]/50">
        {isLoading ? (
          <div className="p-6 space-y-4">
            {[1, 2, 3, 4, 5, 6, 7, 8].map((i) => (
              <div key={i} className="flex items-center gap-3">
                <Skeleton className="h-10 w-10 rounded-full bg-[hsl(var(--layout-border))]" />
                <div className="flex-1">
                  <Skeleton className="h-4 w-3/4 mb-1 bg-[hsl(var(--layout-border))]" />
                  <Skeleton className="h-3 w-1/4 bg-[hsl(var(--layout-border))]" />
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="flex flex-col items-center justify-center py-16 text-center">
            ...
            <p className="text-[hsl(var(--text-secondary))] text-sm mt-1">Activity will appear here when tasks and projects are updated</p>
          </div>
        ) : (
          <div className="divide-y divide-[hsl(var(--layout-border))]/50">
            {Object.entries(groupedActivities).map(([dateKey, activities]) => (
              <div key={dateKey}>
                {/* Date Header */}
                <div className="px-6 py-3 bg-[hsl(var(--layout-card))]/30 border-b border-[hsl(var(--layout-border))]/50">
                  <p className="text-sm font-medium text-[hsl(var(--text-secondary))]">{getDateLabel(dateKey)}</p>
                </div>

                {/* Activities for this date */}
                <div className="divide-y divide-[hsl(var(--layout-border))]/30">
                  {activities.map((activity) => (
                    <div
                      key={activity.id}
                      className="flex items-center gap-4 px-6 py-4 hover:bg-[hsl(var(--layout-card))]/50 cursor-pointer transition-colors"
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 text-[hsl(var(--text-secondary))] text-xs shrink-0">

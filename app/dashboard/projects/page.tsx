...
export default function ProjectsPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      default:
        return (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold text-[hsl(var(--text-secondary))] bg-[hsl(var(--layout-border))]/20">
            {status}
            <span className="w-1.5 h-1.5 rounded-full bg-[hsl(var(--text-secondary))]"></span>
          </span>
        );
    }
  };
    ...
    <div className="min-h-full space-y-6">
        ...
        <div className="bg-[hsl(var(--layout-card))] rounded-xl p-4 border border-[hsl(var(--layout-border))]/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-[hsl(var(--text-primary))]">{projects.length}</p>
            </div>
          </div>
        </div>
        <div className="bg-[hsl(var(--layout-card))] rounded-xl p-4 border border-[hsl(var(--layout-border))]/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-[hsl(var(--text-primary))]">{activeProjects}</p>
            </div>
          </div>
        </div>
        <div className="bg-[hsl(var(--layout-card))] rounded-xl p-4 border border-[hsl(var(--layout-border))]/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-[hsl(var(--text-primary))]">{completedProjects}</p>
            </div>
          </div>
        </div>
        <div className="bg-[hsl(var(--layout-card))] rounded-xl p-4 border border-[hsl(var(--layout-border))]/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-[hsl(var(--text-primary))]">{totalTasks}</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor="name" className="text-[hsl(var(--text-primary))]">Project Name *</Label>
                <Input
                  id="name"
              ...
              <div className="space-y-2">
                <Label htmlFor="department" className="text-[hsl(var(--text-primary))]">Department *</Label>
                <select
                  id="department"
                ...
                >
                  ...
                  {departments.map((dept) => (
                    <option key={dept.id} value={dept.id} className="bg-[hsl(var(--layout-bg))]">
                      {dept.name}
                    </option>
                  ))}
                </select>
      ...
      {editingProject && (
        <div className="bg-[hsl(var(--layout-card))] rounded-xl border border-[hsl(var(--layout-border))]/50 p-4 sm:p-6">
          ...
          <form onSubmit={handleUpdate} className="space-y-4">
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor="edit-name" className="text-[hsl(var(--text-primary))]">Project Name *</Label>
                <Input
                  id="edit-name"
              ...
              <div className="space-y-2">
                <Label htmlFor="edit-status" className="text-[hsl(var(--text-primary))]">Status</Label>
                <select
                  id="edit-status"
            ...
            <div className="space-y-2">
              <Label htmlFor="edit-description" className="text-[hsl(var(--text-primary))]">Description</Label>
              <Input
                id="edit-description"
      ...
      {isLoading ? (
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div key={i} className="bg-[hsl(var(--layout-card))] rounded-xl border border-[hsl(var(--layout-border))]/50 p-5">
              <div className="flex items-start justify-between mb-3">
                <div>
                  <Skeleton className="h-5 w-32 bg-[hsl(var(--layout-border))] mb-2" />
                  <Skeleton className="h-4 w-24 bg-[hsl(var(--layout-border))]" />
                </div>
                <Skeleton className="h-5 w-16 bg-[hsl(var(--layout-border))] rounded" />
              </div>
              <Skeleton className="h-4 w-full bg-[hsl(var(--layout-border))] mb-4" />
              <Skeleton className="h-2 w-full bg-[hsl(var(--layout-border))] mb-4" />
        ...
        <div className="bg-[hsl(var(--layout-card))] rounded-xl border border-[hsl(var(--layout-border))]/50 p-12">
          <div className="flex flex-col items-center justify-center text-center">
            <div className="w-16 h-16 rounded-full bg-[hsl(var(--layout-card))] flex items-center justify-center mb-4">
              <FolderKanban className="h-8 w-8 text-[hsl(var(--text-secondary))]" />
            </div>
          ...
          {filteredProjects.map((project) => {
            ...
            return (
              <ContextMenu key={project.id}>
                <ContextMenuTrigger>
                  <div
                    className="bg-[hsl(var(--layout-card))] rounded-xl border border-[hsl(var(--layout-border))]/50 p-5 hover:bg-[hsl(var(--layout-card-hover))] cursor-pointer transition-all duration-200 hover:border-[hsl(var(--layout-border))]"
                  ...
                  >
                    ...
                    <div className="mb-4">
                      ...
                      <Progress value={progress} className="h-1.5 bg-[hsl(var(--layout-border))]" />
                    </div>
